...
(async()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'tech-hub-native-'));const c=loadConfig(dir);c.host='127.0.0.1';fs.writeFileSync(path.join(dir,'config.json'),JSON.stringify(c));
 ...
 try{let state;for(let i=0;i<100;i++){try{state=await fetch('http://127.0.0.1:8700/api/status',{signal:AbortSignal.timeout(500)}).then(r=>r.json());if(state.services.every(s=>s.state==='running'))break;}catch{}await new Promise(r=>setTimeout(r,300));}assert(state?.services.every(s=>s.state==='running'));console.log('PASS: native Mac host starts all six packaged services.');}
 finally{app.kill('SIGTERM');for(let i=0;i<40;i++){try{await fetch('http://127.0.0.1:8700/api/status',{signal:AbortSignal.timeout(300)});}catch{break;}await new Promise(r=>setTimeout(r,300));}for(const port of [8700,8701,8702,8703,8704,8705,8706,18701,18702,18703,18704,18705,18706])await assert.rejects(fetch(`http://127.0.0.1:${port}`,{signal:AbortSignal.timeout(500)}));console.log('PASS: parent termination cleans up all thirteen listeners.');fs.rmSync(dir,{recursive:true,force:true});}
