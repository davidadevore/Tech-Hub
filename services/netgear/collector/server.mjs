...
const defaults={subnet:'10.1.0.0/24',sourceAddress:'',username:'dashboard',securityLevel:'authPriv',authProtocol:'sha512',authKey:'',privProtocol:'aes',privKey:'',webUsername:'admin',webPassword:'',port:8787,bindAddress:'0.0.0.0',pollSeconds:30};
...
async function inspect(sw){
 const bases=[O.ifDescr,O.ifType,O.ifOper,O.inDiscards,O.inErrors,O.outDiscards,O.outErrors,O.ifName,O.ifHCInOctets,O.ifHCOutOctets,O.ifHighSpeed,O.ifAlias,O.bridgeMap,O.pvid,O.vlanName,O.fdbPort,O.neighborMac,O.lldpName,O.lldpPortDesc,O.lldpMgmt,O.poeDetection,O.opticalRxLegacy,O.opticalRx,O.vlanEgress,O.vlanUntagged];
 ...
 const [fdbRows,neighborRows,lldpRows,lldpDescRows,lldpMgmtRows,poeRows,opticalLegacyRows,opticalRows,vlanEgressRows,vlanUntaggedRows]=result.slice(15),ifToPvid={},bridgeToIf={},ifToBridge={};
...
function saveConfig(next){hosts(next.subnet);if(!next.username?.trim())throw new Error('SNMPv3 username is required');const securityLevel=securityLevelNames.includes(next.securityLevel)?next.securityLevel:config.securityLevel,authKey=next.authKey?.trim()||config.authKey,privKey=next.privKey?.trim()||config.privKey,webPassword=next.webPassword||config.webPassword;if(securityLevel!=='noAuthNoPriv'&&authKey.length<8)throw new Error('An authentication key of at least 8 characters is required for this security level');if(securityLevel==='authPriv'&&privKey.length<8)throw new Error('An encryption key of at least 8 characters is required for authentication + encryption');const oldBind=config.bindAddress;config={...config,subnet:next.subnet.trim(),sourceAddress:(next.sourceAddress||'').trim(),username:next.username.trim(),securityLevel,authProtocol:'sha512',authKey,privProtocol:'aes',privKey,webUsername:(next.webUsername||'admin').trim(),webPassword,bindAddress:managed?'127.0.0.1':(next.bindAddress||'0.0.0.0').trim(),pollSeconds:Math.max(10,Number(next.pollSeconds)||30)};const lines=[`SNMP_SUBNET=${config.subnet}`,`SNMP_SOURCE_ADDRESS=${config.sourceAddress}`,`SNMP_USERNAME=${config.username}`,`SNMP_SECURITY_LEVEL=${config.securityLevel}`,`SNMP_AUTH_PROTOCOL=${config.authProtocol}`,`SNMP_AUTH_KEY=${config.authKey}`,`SNMP_PRIV_PROTOCOL=${config.privProtocol}`,`SNMP_PRIV_KEY=${config.privKey}`,`NETGEAR_WEB_USERNAME=${config.webUsername}`,`NETGEAR_WEB_PASSWORD=${config.webPassword}`,`COLLECTOR_PORT=${config.port}`,`SERVER_BIND_ADDRESS=${config.bindAddress}`,`POLL_SECONDS=${config.pollSeconds}`];fs.writeFileSync(envPath,lines.join('\n')+'\n',{mode:0o600});state={...state,status:'starting',subnet:config.subnet,error:null};scan();return oldBind!==config.bindAddress}
const handleRequest=async(req,res)=>{
 if(req.method==='OPTIONS')return send(res,204,{});
 const url=new URL(req.url||'/',`http://${req.headers.host||'localhost'}`);
 if(url.pathname==='/api/switches')return send(res,200,{...state,dashboardAddress:dashboardAddress(),dashboardPort:Number(process.env.TECH_HUB_PUBLIC_PORT||3000)});
